#!/usr/bin/perl -w -- 
#
# generated by wxGlade 1.1.1 on Sat Feb 28 01:02:48 2026
#
# To get wxPerl visit http://www.wxperl.it
#

use strict;
use warnings;

use Wx;
use threads;
use Thread::Queue;
use File::Spec;
use Cwd;

# begin wxGlade: dependencies
# end wxGlade

# begin wxGlade: extracode
# end wxGlade

package MyFrame;

use Wx qw[:everything];
use base qw(Wx::Frame);
use strict;

sub new {
    my( $self, $parent, $id, $title, $pos, $size, $style, $name ) = @_;
    $parent = undef              unless defined $parent;
    $id     = -1                 unless defined $id;
    $title  = ""                 unless defined $title;
    $pos    = wxDefaultPosition  unless defined $pos;
    $size   = wxDefaultSize      unless defined $size;
    $name   = ""                 unless defined $name;

    # begin wxGlade: MyFrame::new
    $style = wxCAPTION|wxCLIP_CHILDREN|wxCLOSE_BOX|wxMAXIMIZE_BOX|wxMINIMIZE_BOX|wxSYSTEM_MENU
        unless defined $style;

    $self = $self->SUPER::new( $parent, $id, $title, $pos, $size, $style, $name );
    $self->SetSize(Wx::Size->new(800, 596));
    $self->SetTitle("wxPerl GUI Helper for Windows By Perl-Guilds.net");
    
    

    # Menu Bar

    $self->{frame_menubar} = Wx::MenuBar->new();
    my $wxglade_tmp_menu;
    $wxglade_tmp_menu = Wx::Menu->new();
    $self->{Open} = $wxglade_tmp_menu->Append(wxID_ANY, "Open (.pl) ...", "");
    $self->{Exit} = $wxglade_tmp_menu->Append(wxID_ANY, "Exit", "");
    $self->{frame_menubar}->Append($wxglade_tmp_menu, "File");
    $wxglade_tmp_menu = Wx::Menu->new();
    $self->{Aboutt} = $wxglade_tmp_menu->Append(wxID_ANY, "About", "");
    $self->{frame_menubar}->Append($wxglade_tmp_menu, "Help");
    $self->SetMenuBar($self->{frame_menubar});
    
    # Menu Bar end

    
    $self->{panel_1} = Wx::Panel->new($self, wxID_ANY);
    
    $self->{sizer_1} = Wx::BoxSizer->new(wxVERTICAL);
    
    $self->{notebook_1} = Wx::Notebook->new($self->{panel_1}, wxID_ANY);
    $self->{sizer_1}->Add($self->{notebook_1}, 1, wxEXPAND, 0);
    
    $self->{notebook_1_pane_1} = Wx::Panel->new($self->{notebook_1}, wxID_ANY);
    $self->{notebook_1}->AddPage($self->{notebook_1_pane_1}, "DLL Finder");
    
    $self->{sizer_2} = Wx::FlexGridSizer->new(2, 2, 0, 0);
    
    my $label_1 = Wx::StaticText->new($self->{notebook_1_pane_1}, wxID_ANY, "Source (.pl)");
    $self->{sizer_2}->Add($label_1, 0, wxALIGN_CENTER, 0);
    
    $self->{sizer_4} = Wx::BoxSizer->new(wxHORIZONTAL);
    $self->{sizer_2}->Add($self->{sizer_4}, 1, wxEXPAND, 0);
    
    $self->{perl_script_path} = Wx::TextCtrl->new($self->{notebook_1_pane_1}, wxID_ANY, "");
    $self->{perl_script_path}->SetMinSize(Wx::Size->new(444, 23));
    $self->{sizer_4}->Add($self->{perl_script_path}, 1, wxEXPAND|wxLEFT|wxRIGHT, 6);
    
    $self->{button_browse_pl} = Wx::Button->new($self->{notebook_1_pane_1}, wxID_ANY, "Browse\N{U+2026}");
    $self->{sizer_4}->Add($self->{button_browse_pl}, 0, wxEXPAND, 0);
    
    $self->{button_3} = Wx::Button->new($self->{notebook_1_pane_1}, wxID_ANY, "Find DLLs");
    $self->{button_3}->SetToolTip("Runs pp_autolink to find the DLLs needed");
    $self->{sizer_4}->Add($self->{button_3}, 0, wxEXPAND, 0);
    
    $self->{sizer_3} = Wx::BoxSizer->new(wxVERTICAL);
    $self->{sizer_2}->Add($self->{sizer_3}, 1, wxEXPAND, 0);
    
    $self->{sizer_3}->Add(0, 0, 0, 0, 0);
    
    $self->{sizer_3}->Add(73, 200, 0, 0, 0);
    
    $self->{sizer_3}->Add(0, 0, 0, 0, 0);
    
    $self->{sizer_3}->Add(73, 200, 0, 0, 0);
    
    $self->{sizer_3}->Add(0, 0, 0, 0, 0);
    
    $self->{sizer_3}->Add(0, 0, 0, 0, 0);
    
    $self->{sizer_3}->Add(73, 200, 0, 0, 0);
    
    $self->{txt_cmd_io} = Wx::TextCtrl->new($self->{notebook_1_pane_1}, wxID_ANY, "", wxDefaultPosition, wxDefaultSize, wxTE_BESTWRAP|wxTE_MULTILINE|wxTE_RICH2);
    $self->{txt_cmd_io}->SetMinSize(Wx::Size->new(800, 496));
    $self->{sizer_2}->Add($self->{txt_cmd_io}, 1, wxALL|wxEXPAND, 3);
    
    $self->{notebook_1_MakefileBuilder} = Wx::Panel->new($self->{notebook_1}, wxID_ANY);
    $self->{notebook_1}->AddPage($self->{notebook_1_MakefileBuilder}, "Makefile Builder");
    
    $self->{sizer_mk_root} = Wx::BoxSizer->new(wxVERTICAL);
    
    $self->{sizer_mk_project} = Wx::StaticBoxSizer->new(Wx::StaticBox->new($self->{notebook_1_MakefileBuilder}, wxID_ANY, "Project"), wxVERTICAL);
    $self->{sizer_mk_root}->Add($self->{sizer_mk_project}, 0, wxALL|wxEXPAND, 10);
    
    $self->{sizer_mk_project_grid} = Wx::FlexGridSizer->new(4, 2, 6, 10);
    $self->{sizer_mk_project}->Add($self->{sizer_mk_project_grid}, 0, wxALL|wxEXPAND, 6);
    
    my $label_mk_appname = Wx::StaticText->new($self->{sizer_mk_project}->GetStaticBox(), wxID_ANY, "App name");
    $self->{sizer_mk_project_grid}->Add($label_mk_appname, 0, wxALIGN_CENTER_VERTICAL, 0);
    
    $self->{txt_mk_appname} = Wx::TextCtrl->new($self->{sizer_mk_project}->GetStaticBox(), wxID_ANY, "");
    $self->{sizer_mk_project_grid}->Add($self->{txt_mk_appname}, 1, wxEXPAND, 0);
    
    my $label_mk_srcpl = Wx::StaticText->new($self->{sizer_mk_project}->GetStaticBox(), wxID_ANY, "Source .pl (full path)");
    $self->{sizer_mk_project_grid}->Add($label_mk_srcpl, 0, wxALIGN_CENTER_VERTICAL, 0);
    
    $self->{txt_mk_srcpl} = Wx::TextCtrl->new($self->{sizer_mk_project}->GetStaticBox(), wxID_ANY, "");
    $self->{sizer_mk_project_grid}->Add($self->{txt_mk_srcpl}, 1, wxEXPAND, 0);
    
    my $label_mk_distdir = Wx::StaticText->new($self->{sizer_mk_project}->GetStaticBox(), wxID_ANY, "dist/ output dir");
    $self->{sizer_mk_project_grid}->Add($label_mk_distdir, 0, wxALIGN_CENTER_VERTICAL, 0);
    
    $self->{txt_mk_distdir} = Wx::TextCtrl->new($self->{sizer_mk_project}->GetStaticBox(), wxID_ANY, "");
    $self->{sizer_mk_project_grid}->Add($self->{txt_mk_distdir}, 1, wxEXPAND, 0);
    
    my $label_mk_setup = Wx::StaticText->new($self->{sizer_mk_project}->GetStaticBox(), wxID_ANY, "setup target (.iss)");
    $self->{sizer_mk_project_grid}->Add($label_mk_setup, 0, wxALIGN_CENTER_VERTICAL, 0);
    
    $self->{txt_mk_iss} = Wx::TextCtrl->new($self->{sizer_mk_project}->GetStaticBox(), wxID_ANY, "");
    $self->{sizer_mk_project_grid}->Add($self->{txt_mk_iss}, 1, wxEXPAND, 0);
    
    $self->{sizer_mk_wxpar} = Wx::StaticBoxSizer->new(Wx::StaticBox->new($self->{notebook_1_MakefileBuilder}, wxID_ANY, "wxpar options"), wxVERTICAL);
    $self->{sizer_mk_root}->Add($self->{sizer_mk_wxpar}, 0, wxBOTTOM|wxEXPAND|wxLEFT|wxRIGHT, 10);
    
    $self->{sizer_mk_wxpar_row1} = Wx::BoxSizer->new(wxHORIZONTAL);
    $self->{sizer_mk_wxpar}->Add($self->{sizer_mk_wxpar_row1}, 0, wxALL|wxEXPAND, 6);
    
    $self->{chk_mk_gui} = Wx::CheckBox->new($self->{sizer_mk_wxpar}->GetStaticBox(), wxID_ANY, "--gui");
    $self->{chk_mk_gui}->SetValue(1);
    $self->{sizer_mk_wxpar_row1}->Add($self->{chk_mk_gui}, 0, wxALIGN_CENTER_VERTICAL|wxRIGHT, 0);
    
    $self->{chk_mk_build_debug} = Wx::CheckBox->new($self->{sizer_mk_wxpar}->GetStaticBox(), wxID_ANY, "Build DEBUG target too");
    $self->{chk_mk_build_debug}->SetValue(1);
    $self->{sizer_mk_wxpar_row1}->Add($self->{chk_mk_build_debug}, 0, wxALIGN_CENTER_VERTICAL|wxRIGHT, 10);
    
    $self->{sizer_mk_wxpar_row1_fill} = Wx::BoxSizer->new(wxHORIZONTAL);
    $self->{sizer_mk_wxpar_row1}->Add($self->{sizer_mk_wxpar_row1_fill}, 1, wxEXPAND, 0);
    
    $self->{sizer_mk_wxpar_row1_fill}->Add(0, 0, 0, 0, 0);
    
    my $label_mk_extra = Wx::StaticText->new($self->{sizer_mk_wxpar}->GetStaticBox(), wxID_ANY, "Extra flags");
    $self->{sizer_mk_wxpar_row1}->Add($label_mk_extra, 0, wxALIGN_CENTER_VERTICAL, 0);
    
    $self->{txt_mk_extra_flags} = Wx::TextCtrl->new($self->{sizer_mk_wxpar}->GetStaticBox(), wxID_ANY, "");
    $self->{sizer_mk_wxpar_row1}->Add($self->{txt_mk_extra_flags}, 1, wxEXPAND|wxLEFT, 6);
    
    $self->{sizer_mk_dlls} = Wx::StaticBoxSizer->new(Wx::StaticBox->new($self->{notebook_1_MakefileBuilder}, wxID_ANY, "DLL links"), wxVERTICAL);
    $self->{sizer_mk_root}->Add($self->{sizer_mk_dlls}, 1, wxBOTTOM|wxEXPAND|wxLEFT|wxRIGHT, 10);
    
    $self->{sizer_mk_dlls_grid} = Wx::FlexGridSizer->new(1, 2, 0, 10);
    $self->{sizer_mk_dlls}->Add($self->{sizer_mk_dlls_grid}, 0, wxALL|wxEXPAND, 6);
    
    my $label_mk_cbin = Wx::StaticText->new($self->{sizer_mk_dlls}->GetStaticBox(), wxID_ANY, "CBIN (optional base dir)");
    $self->{sizer_mk_dlls_grid}->Add($label_mk_cbin, 0, wxALIGN_CENTER_VERTICAL, 0);
    
    $self->{txt_mk_cbin} = Wx::TextCtrl->new($self->{sizer_mk_dlls}->GetStaticBox(), wxID_ANY, "c:\\sw\\pdl\\c\\bin\\");
    $self->{sizer_mk_dlls_grid}->Add($self->{txt_mk_cbin}, 1, wxEXPAND, 0);
    
    $self->{txt_mk_dll_list} = Wx::TextCtrl->new($self->{sizer_mk_dlls}->GetStaticBox(), wxID_ANY, "c:/sw/pdl/c/bin/libhdf__.dll\nc:/sw/pdl/c/bin/liblzma-5__.dll\nc:/sw/pdl/c/bin/libiconv-2__.dll\nc:/sw/pdl/c/bin/libmfhdf__.dll\nc:/sw/pdl/c/bin/libhdf5-0__.dll\nc:/sw/pdl/c/bin/libjpeg-9__.dll\nc:/sw/pdl/c/bin/libssl-3-x64__.dll\nc:/sw/pdl/c/bin/libsz-2__.dll\nc:/sw/pdl/c/bin/libcrypto-3-x64__.dll\nc:/sw/pdl/c/bin/zlib1__.dll\nc:/sw/pdl/c/bin/libnetcdf-19__.dll\nc:/sw/pdl/c/bin/libhdf5_hl-0__.dll\nc:/sw/pdl/c/bin/libxml2-2__.dll", wxDefaultPosition, wxDefaultSize, wxHSCROLL|wxTE_MULTILINE);
    $self->{sizer_mk_dlls}->Add($self->{txt_mk_dll_list}, 1, wxALL|wxEXPAND, 6);
    
    my $label_mk_dll_hint = Wx::StaticText->new($self->{sizer_mk_dlls}->GetStaticBox(), wxID_ANY, "One DLL per line. (Hint: get list from DLL Finder tab.)");
    $self->{sizer_mk_dlls}->Add($label_mk_dll_hint, 0, wxALL|wxEXPAND, 6);
    
    $self->{sizer_mk_inno} = Wx::StaticBoxSizer->new(Wx::StaticBox->new($self->{notebook_1_MakefileBuilder}, wxID_ANY, "Installer (setup target)"), wxVERTICAL);
    $self->{sizer_mk_root}->Add($self->{sizer_mk_inno}, 0, wxBOTTOM|wxEXPAND|wxLEFT|wxRIGHT, 10);
    
    $self->{sizer_mk_inno_grid} = Wx::FlexGridSizer->new(1, 2, 0, 10);
    $self->{sizer_mk_inno}->Add($self->{sizer_mk_inno_grid}, 0, wxALL|wxEXPAND, 6);
    
    my $label_mk_iscc = Wx::StaticText->new($self->{sizer_mk_inno}->GetStaticBox(), wxID_ANY, "ISCC.exe (full path)");
    $self->{sizer_mk_inno_grid}->Add($label_mk_iscc, 0, wxALIGN_CENTER_VERTICAL, 0);
    
    $self->{txt_mk_iscc} = Wx::TextCtrl->new($self->{sizer_mk_inno}->GetStaticBox(), wxID_ANY, "C:\\Users\\user\\AppData\\Local\\Programs\\Inno Setup 6\\ISCC.exe");
    $self->{sizer_mk_inno_grid}->Add($self->{txt_mk_iscc}, 1, wxEXPAND, 0);
    
    $self->{sizer_mk_preview} = Wx::StaticBoxSizer->new(Wx::StaticBox->new($self->{notebook_1_MakefileBuilder}, wxID_ANY, "Makefile preview"), wxVERTICAL);
    $self->{sizer_mk_root}->Add($self->{sizer_mk_preview}, 1, wxBOTTOM|wxEXPAND|wxLEFT|wxRIGHT, 10);
    
    $self->{txt_mk_preview} = Wx::TextCtrl->new($self->{sizer_mk_preview}->GetStaticBox(), wxID_ANY, "# Preview will be generated by the Perl driver later.", wxDefaultPosition, wxDefaultSize, wxHSCROLL|wxTE_MULTILINE|wxTE_READONLY);
    $self->{sizer_mk_preview}->Add($self->{txt_mk_preview}, 1, wxALL|wxEXPAND, 6);
    
    $self->{notebook_1_pane_2} = Wx::Panel->new($self->{notebook_1}, wxID_ANY);
    $self->{notebook_1}->AddPage($self->{notebook_1_pane_2}, "Installer Configurator");
    
    $self->{sizer_inno_main} = Wx::BoxSizer->new(wxVERTICAL);
    
    $self->{sizer_iscc} = Wx::StaticBoxSizer->new(Wx::StaticBox->new($self->{notebook_1_pane_2}, wxID_ANY, "ISCC.exe"), wxHORIZONTAL);
    $self->{sizer_inno_main}->Add($self->{sizer_iscc}, 0, wxALL|wxEXPAND, 10);
    
    $self->{txt_iscc_path} = Wx::TextCtrl->new($self->{sizer_iscc}->GetStaticBox(), wxID_ANY, "C:\\Program Files (x86)\\Inno Setup 6\\ISCC.exe");
    $self->{sizer_iscc}->Add($self->{txt_iscc_path}, 1, wxEXPAND|wxRIGHT, 6);
    
    $self->{btn_browse_iscc} = Wx::Button->new($self->{sizer_iscc}->GetStaticBox(), wxID_ANY, "Browse\N{U+2026}");
    $self->{sizer_iscc}->Add($self->{btn_browse_iscc}, 0, 0, 0);
    
    $self->{sizer_inno_opts} = Wx::StaticBoxSizer->new(Wx::StaticBox->new($self->{notebook_1_pane_2}, wxID_ANY, "Options"), wxVERTICAL);
    $self->{sizer_inno_main}->Add($self->{sizer_inno_opts}, 0, wxBOTTOM|wxLEFT|wxRIGHT, 10);
    
    $self->{chk_inno_quiet} = Wx::CheckBox->new($self->{sizer_inno_opts}->GetStaticBox(), wxID_ANY, "Quiet compile (/Q)");
    $self->{chk_inno_quiet}->SetValue(1);
    $self->{sizer_inno_opts}->Add($self->{chk_inno_quiet}, 0, wxALL, 4);
    
    $self->{chk_inno_open_output} = Wx::CheckBox->new($self->{sizer_inno_opts}->GetStaticBox(), wxID_ANY, "Open output folder after build");
    $self->{sizer_inno_opts}->Add($self->{chk_inno_open_output}, 0, wxALL, 4);
    
    $self->{chk_inno_run_installer} = Wx::CheckBox->new($self->{sizer_inno_opts}->GetStaticBox(), wxID_ANY, "Run installer after build");
    $self->{sizer_inno_opts}->Add($self->{chk_inno_run_installer}, 0, wxALL, 4);
    
    $self->{sizer_inno_notes} = Wx::StaticBoxSizer->new(Wx::StaticBox->new($self->{notebook_1_pane_2}, wxID_ANY, "Notes / Defines (optional)"), wxVERTICAL);
    $self->{sizer_inno_main}->Add($self->{sizer_inno_notes}, 1, wxBOTTOM|wxEXPAND|wxLEFT|wxRIGHT, 10);
    
    $self->{txt_inno_defines} = Wx::TextCtrl->new($self->{sizer_inno_notes}->GetStaticBox(), wxID_ANY, "", wxDefaultPosition, wxDefaultSize, wxHSCROLL|wxTE_MULTILINE);
    $self->{sizer_inno_notes}->Add($self->{txt_inno_defines}, 1, wxALL|wxEXPAND, 6);
    
    $self->{notebook_1_pane_3} = Wx::Panel->new($self->{notebook_1}, wxID_ANY);
    $self->{notebook_1}->AddPage($self->{notebook_1_pane_3}, "Help");
    
    $self->{notebook_1_pane_2}->SetSizer($self->{sizer_inno_main});
    
    $self->{sizer_mk_inno_grid}->AddGrowableCol(1);
    
    $self->{sizer_mk_dlls_grid}->AddGrowableCol(1);
    
    $self->{sizer_mk_project_grid}->AddGrowableRow(0);
    $self->{sizer_mk_project_grid}->AddGrowableCol(1);
    
    $self->{notebook_1_MakefileBuilder}->SetSizer($self->{sizer_mk_root});
    
    $self->{notebook_1_pane_1}->SetSizer($self->{sizer_2});
    
    $self->{panel_1}->SetSizer($self->{sizer_1});
    
    $self->Layout();
    Wx::Event::EVT_MENU($self, $self->{Open}->GetId, $self->can('select_perl_script'));
    Wx::Event::EVT_MENU($self, $self->{Exit}->GetId, $self->can('DoQuit'));
    Wx::Event::EVT_MENU($self, $self->{Aboutt}->GetId, $self->can('show_license_dialog'));
    Wx::Event::EVT_BUTTON($self, $self->{button_browse_pl}->GetId, $self->can('select_perl_script'));
    Wx::Event::EVT_BUTTON($self, $self->{button_3}->GetId, $self->can('run_pp_autolink'));

    # end wxGlade

    # ----- background worker comms -----
    $self->{_q}     = Thread::Queue->new();
    $self->{_timer} = Wx::Timer->new($self, wxID_ANY);
    Wx::Event::EVT_TIMER($self, $self->{_timer}, sub { $self->_drain_worker_queue });
    $self->{_timer}->Start(100); # 100ms poll
    # -----------------------------------
    return $self;

}

sub _append_io {
    my ($self, $text) = @_;
    return unless defined $text && length $text;

    $self->{txt_cmd_io}->AppendText($text);
    $self->{txt_cmd_io}->ShowPosition($self->{txt_cmd_io}->GetLastPosition);
}

sub _drain_worker_queue {
    my ($self) = @_;
    my $q = $self->{_q} or return;

    while (defined(my $msg = $q->dequeue_nb)) {
        if (ref($msg) eq 'HASH' && ($msg->{type} // '') eq 'DONE') {
            $self->_append_io("\n[done] exit_code=$msg->{exit_code}\n");
            $self->{button_3}->Enable(1);
            next;
        }
        $self->_append_io($msg);
    }
}

sub _get_perl_root {
    my $perl_path = $^X;  # full path to running perl.exe
    my ($volume, $dirs, $file) = File::Spec->splitpath($perl_path);
    my @parts = File::Spec->splitdir($dirs);
    
    # Remove \perl\bin\
    pop @parts; # bin
    pop @parts; # perl
    
    my $perl_root = File::Spec->catdir($volume, @parts);
    return $perl_root;
}

sub DoQuit {
    my ($self, $event) = @_;
    # wxGlade: MyFrame::DoQuit <event_handler>
    # end wxGlade
    $self->Close;
}

sub show_license_dialog {
    my ($self, $event) = @_;
    # wxGlade: MyFrame::show_license_dialog <event_handler>
    warn "Event handler (show_license_dialog) not implemented";
    $event->Skip;
    # end wxGlade
}


sub select_perl_script {
    my ($self, $event) = @_;
    # wxGlade: MyFrame::select_perl_script <event_handler>
    $event->Skip;
    # end wxGlade
    my $dlg = Wx::FileDialog->new(
        $self,
        "Select Perl Script",
        "", "",
        "Perl files (*.pl)|*.pl|All files (*.*)|*.*",
        Wx::wxFD_OPEN() | Wx::wxFD_FILE_MUST_EXIST()
    );

    if ($dlg->ShowModal == Wx::wxID_OK()) {
        my $path = $dlg->GetPath;  # full path
        $self->{perl_script_path}->SetValue($path);
        $self->_append_io("Selected: $path\n");
    }

    $dlg->Destroy;
}


sub run_pp_autolink {
    my ($self, $event) = @_;
    # wxGlade: MyFrame::run_pp_autolink <event_handler>
    # end wxGlade

    my $script = $self->{perl_script_path}->GetValue // '';
    $script =~ s/^\s+|\s+$//g;
    my $exe = $script;
    $exe =~ s/\.pl/\.exe/g;

    $self->_append_io("\n[debug] Find DLLs clicked\n");

    if (!$script) {
        $self->_append_io("[error] No .pl selected.\n");
        return;
    }
    if (!-f $script) {
        $self->_append_io("[error] File not found: $script\n");
        return;
    }

    $script = Cwd::abs_path($script) || $script;
    $self->_append_io("[debug] script resolved to: $script\n");

    $self->{button_3}->Enable(0);
    $self->_append_io("=== Starting pp autolink scan ===\n");

    my $q = $self->{_q};

    threads->create(sub {
        my ($script_path, $qref) = @_;

        $qref->enqueue("[worker] thread started\n");

        # Check pp is runnable
        my $pp_ver = `pp --version 2>&1`;
        my $pp_rc  = $? >> 8;
        $qref->enqueue("[worker] pp --version rc=$pp_rc\n$pp_ver\n");

        if ($pp_rc != 0) {
            $qref->enqueue("[worker][error] 'pp_autolink' command failed. Is PAR::Packer installed and on PATH?\n");
            $qref->enqueue({ type => 'DONE', exit_code => $pp_rc });
            return;
        }

        my @cmd = qw/pp_autolink -o tmp.exe/;
        push @cmd, $script_path;

        my $pretty = join(' ', map { $_ =~ /\s/ ? qq{"$_"} : $_ } @cmd);
        $qref->enqueue("[worker] cmd: $pretty\n");

        my $cmdline = $pretty . " 2>&1";

        my $exit = 0;
        if (open(my $fh, "$cmdline |")) {
            $qref->enqueue("[worker] pipe opened, reading output...\n");

            my $line_count = 0;
            my $CMD_line = "";
            while (my $line = <$fh>) {
                $line_count++;
                $qref->enqueue($line);
                if ($line =~ m/^CMD:/) {
                  $CMD_line = $line;
                }
            }

            my $perl_path = $^X;  # full path to running perl.exe
            
            my ($volume, $dirs, $file) = File::Spec->splitpath($perl_path);
            my @parts = File::Spec->splitdir($dirs);
            
            # Remove \perl\bin\
            pop @parts;   # bin
            pop @parts;   # perl
            pop @parts;
            push @parts, "c";
            my $filter = join("/", @parts);
            my $perl_root = lc File::Spec->catdir($volume, @parts);
            my @cmd = split / /, $CMD_line;
            my @filtered_libs = grep { m/\Q$filter\E/ } @cmd;
            
            $qref->enqueue("\n\n=== Perl moduled DLLs to include ===n\n");
            my $wxpar = "wxpar -o $exe";
            foreach my $lib (@filtered_libs) {
              $wxpar .= " --link $lib";
              $qref->enqueue(" --link $lib \n");
            }
            $wxpar .= " $script --gui";
            
            $qref->enqueue("\n[worker] pp_autolink process completed...");

            $qref->enqueue("\n\n=== 'wxpar' command (may be used in a Makefile) ===n\n");
            
            $wxpar =~ s/\\/\//g;
            
            $qref->enqueue("$wxpar\n\n\n");

            close($fh);
            $exit = $? >> 8;
            $qref->enqueue("[worker] pipe closed, lines=$line_count, rc=$exit\n");
        } else {
            $qref->enqueue("[worker][error] failed to run command pipe: $!\n");
            $exit = 127;
        }

        $qref->enqueue({ type => 'DONE', exit_code => $exit });
        return;
    }, $script, $q)->detach();

    $self->_append_io("[debug] worker thread launched\n");
}

# end of class MyFrame

1;

package MyApp;

use base qw(Wx::App);
use strict;

sub OnInit {
    my( $self ) = shift;

    Wx::InitAllImageHandlers();

    my $frame = MyFrame->new();

    $self->SetTopWindow($frame);
    $frame->Show(1);

    return 1;
}
# end of class MyApp

package main;

my $app = MyApp->new();
$app->MainLoop();
